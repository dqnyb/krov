(function() {
  ...
  function onUserRequest(message) {
      ...
      chatMessages.appendChild(messageElement);
      chatMessages.scrollTop = chatMessages.scrollHeight;
      console.log("final = ",onboardingStep);
      ...
      if (onboardingStep == 4) {
        ...
        if (onboardingStep === 14) {
          ...
          fetch("https://krov.onrender.com/final_stage", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ name: userName, interests: userInterests, message: message, language: window.language })
          })
        ...
        if (onboardingStep === 12) {
          ...
          fetch("https://krov.onrender.com/check_resp", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ name: userName, interests: userInterests, message: message, language: window.language })
          })
        ...
        if (onboardingStep === 13) {
          ...
          fetch("https://krov.onrender.com/ai_mai_comandat", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ name: userName, interests: userInterests, message: message, language: window.language })
          })
            ...
            .then(data => {
              setTimeout(() => {
                ...
                if (data.reply.includes("Ne bucurăm enorm să aflăm că") || data.reply.includes("Мы очень рады узнать")){
                  onboardingStep = 7;
                } else if (data.reply.includes("Nu este nicio problemă") || data.reply.includes("Нет проблем, каждое начало важно"))
                  onboardingStep = 11;
              }, 1000);
            })
            .catch(err => {
        ...
        if (onboardingStep === 15) {
          ...
          fetch("https://krov.onrender.com/numar_de_telefon_final", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ name: userName, interests: userInterests, message: message, language: window.language })
          })
